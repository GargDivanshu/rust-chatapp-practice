...
function subscribe(uri) {
  ...
  function connect(uri) {
    const events = new EventSource(uri);  // eventsource obj establishes conn with server 
     // and listens for new messages

    events.addEventListener("message", (ev) => {
      console.log("raw data", JSON.stringify(ev.data)); //then it is passed into json 
      console.log("decoded data", JSON.stringify(JSON.parse(ev.data))); // and appended to dom 
...
function init() {
  ...
  newMessageForm.addEventListener("submit", (e) => {
    ...
    if (STATE.connected) {
      fetch("/message", {
        method: "POST",  //to send msg this is the post req which is dispathced 
